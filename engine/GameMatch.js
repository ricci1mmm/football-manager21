...
export class GameMatch {
 constructor(options={}){
  ...
  this.coach={
   home:new CoachAI(this.engine.home),
   away:new CoachAI(this.engine.away)
  };
 ...
 updateBall(){
  if(this.ballAI) this.ballAI.update();
 }

 updateStamina(){
  this.players.forEach(player=>this.stamina.update(player));
 }

 updateCoaches(){
  this.coach.home.analyze(this.state);
  this.coach.away.analyze(this.state);
 }
